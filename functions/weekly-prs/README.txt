...
| File                 | Description                                                                     |
|----------------------|---------------------------------------------------------------------------------|
| `weekly-prs.sh`      | Core logic script — run through `gift weekly-prs`, or from the terminal        |
| `config.schema.json` | The settings this function has, and their defaults — see `functions.weekly-prs` in config.json |
...
```bash
gift weekly-prs [--repos=owner/repo1,owner/repo2] [--author=login] [--weeks=N] [-v]
```

(`gift weekly` is enough of the name, and the script also runs on its own:
`./weekly-prs.sh`.)
...
this script. Set them once under `functions.weekly-prs` in config.json —
...
gift weekly-prs --repos=myorg/myrepo --weeks=0

# Two weeks ago, someone else's PRs, verbose output
gift weekly-prs --repos=myorg/myrepo --author=octocat --weeks=2 -v
```
