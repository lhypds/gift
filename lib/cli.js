...
//
// <function> is the name of a folder in functions/ — or `serve`, `stop` and
// `hook`, which drive the webhooks server in webhooks/ — or any unique prefix.
...
const BUILTINS = {
    ...
    hook: 'List, create and delete the server\'s hooks, and read its log.',
...
// the built-ins: all three are about the server in webhooks/.
const WEBHOOK_NAMES = ['serve', 'stop', 'hook'];
...
function printFunctionHelp(token) {
    ...
    if (result.builtin) {
        // `hook` is a command with commands of its own, so it has real help.
        if (result.builtin === 'hook') {
            hook.usage();
            return 0;
        }
...
async function main(argv) {
    ...
    // reads webhooks/.env, the same settings the server starts with.
    if (result.builtin) {
        if (result.builtin === 'hook') env.loadFor(WEBHOOK_DIR);
