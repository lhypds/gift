...
// webhook server in server/ — or any unique prefix of one. `gift list` runs
...
const SERVER_DIR = path.join(commands.ROOT, 'server');

// The webhook server is not one of the commands/ folders — it is a service that
// happens to be started from this CLI — so it is wired up here by name.
const SERVE = {
    name: 'serve',
    description: 'Start the GitHub webhook server (server/).',
    dir: SERVER_DIR,
    entry: path.join(SERVER_DIR, 'server.js'),
};
...
function printHelp() {
    ...
    console.log('');
    console.log('Commands are the folder names in commands/, and any unique prefix works:');
    console.log('  gift list    ->  list-weekly-prs');
    console.log('  gift recur   ->  recursively-pull-repos');
    console.log('');
...
async function main(argv) {
    ...
    const result = resolveToken(token);
    if (result.status !== 'ok') return reportResolveFailure(token, result);
    if (result.builtin) return runBuiltin(result.builtin, rest);
