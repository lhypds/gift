...
const BUILTINS = {
    ...
    update: 'Pull the latest gift code.',
...
async function main(argv) {
    ...
    // `log` and `status` read webhooks/.env, the same settings the server starts
    // with — the address to ask, and PM2_NAME to look the process up under.
    if (result.builtin) {
        if (['hook', 'log', 'status'].includes(result.builtin)) env.loadFor(WEBHOOK_DIR);
