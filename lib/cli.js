...
//
// <function> is the name of a folder in functions/ — or `serve` and `stop`,
// which drive the webhooks server in webhooks/ — or any unique prefix of one.
...
function printHelp() {
    ...
    for (const service of Object.values(SERVICE)) {
        console.log(`  ${pad(service.name, width)}  ${service.description}`);
    }
    console.log('');
    console.log('built-in:');
    for (const [name, description] of Object.entries(BUILTINS)) {
        if (SERVICE[name]) continue; // listed above, under its own heading
...
async function main(argv) {
    ...
    // `run` is the exception: it loads for whichever function gets picked.
    if (result.builtin) {
        if (result.builtin !== 'run') env.loadFor();
...
module.exports = { main, BUILTINS, SERVE, STOP, SERVICE, resolveToken };
