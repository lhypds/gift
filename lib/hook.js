...
//   gift hook log [lines]     the tail of the server's log
//
// All four work on webhooks/hooks.json — the file `gift serve` reads at startup
...
const SUBCOMMANDS = {
    ...
    log: `Print the last ${DEFAULT_LOG_LINES} lines of the server log.`,
};
...
function defaultName(repoName, taken) {
    const base = repoName ? `deploy-${repoName.toLowerCase()}` : 'hook';
...
async function createHook(file) {
    ...
    const runAnswer = await askText('Script to run — a path, relative to here is fine', {
        validate: (value) => (value ? null : 'A script is needed; it is what the hook runs.'),
    });
...
function printLog(file, options, positionals) {
    ...
    if (LOG_OFF.includes(String(setting).trim().toLowerCase())) {
        console.error('gift hook: file logging is off, so there is no log to print.');
        console.error('Set GIFT_SERVE_LOG (or "log" in hooks.json) to a file to keep one.');
        return 1;
    }

    const requested = options.lines ?? positionals[0];
    const count = requested === undefined ? DEFAULT_LOG_LINES : Number(requested);
    if (!Number.isInteger(count) || count <= 0) {
        console.error(`gift hook: '${requested}' is not a number of lines`);
        return 2;
    }
    ...
    if (!current) {
        console.error(`gift hook: no log at ${show(logPath)} yet.`);
    ...
    // The header goes to stderr, so `gift hook log > file` holds the log alone.
    console.error(
        older.length
...
function usage() {
    ...
    line('log [lines]', SUBCOMMANDS.log);
    console.log('');
    console.log('options:');
    ...
    console.log(`  --lines=N       How many lines \`log\` prints (default: ${DEFAULT_LOG_LINES})`);
    ...
    console.log('shortened to any unique prefix: `gift hook cr`, `gift hook lo`.');
}
...
async function main(argv) {
    ...
    try {
        switch (command.name) {
            ...
            case 'log':
                return printLog(file, options, rest);
            default:
                return 2;
        }
    } catch (err) {
