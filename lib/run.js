// Run a command folder's entry script with the user's arguments.
'use strict';
...
/**
 * Run a command and resolve with its exit code. The child inherits stdio and
 * the user's working directory — commands like `recursively-pull-repos` act on
 * the directory the user is standing in.
 */
function runCommand(command, args = []) {
    const [file, prefixArgs] = interpreterFor(command.entry);
    ...
    const child = spawn(file, [...prefixArgs, ...args], {
        ...
        env: {
            ...
            GIFT_COMMAND: command.name,
            GIFT_COMMAND_DIR: command.dir,
    ...
    // how a long-running command like `gift serve` gets stopped by a service
    ...
    return new Promise((resolve, reject) => {
        child.on('error', (err) => {
            stopForwarding();
            if (err.code === 'ENOENT') {
                reject(new Error(`${file}: not found (needed to run '${command.name}')`));
                return;
            }
            reject(err);
        });
