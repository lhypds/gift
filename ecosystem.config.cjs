// PM2 configuration for the gift hooks server, used by ./start.sh:
//
//     pm2 start ecosystem.config.cjs --update-env
//
// PM2_NAME and PORT come from gift's configuration — config.json in this folder
// — so the process name and the listening port are set in one place. Everything
// else the server needs — the webhook secret, the host, the endpoint path, each
// trigger's settings — serve.js reads from the same place at startup, which
// keeps the secret out of the PM2 process list.
const config = require('./utils/config.js');

const PM2_NAME = config.get('PM2_NAME', 'gift');
